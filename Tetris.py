...
def clear_rows(grid, locked):
    ...
    if inc > 0:
        for key in sorted(list(locked), key = lambda x: x[1])[::-1]:
            x, y = key 
            if y < ind:
                newKey = (x, y + inc) #y + inc shifts all rows down plus adds a clear row
...
def update_score(nscore):
    ...
    with open('C:\\Users\\czwat\\OneDrive\\Documents\\scores.txt', 'w') as f:
        if int(score) > nscore:
            f.write(str(score))
        else:
            f.write(str(nscore))
...
def max_score():
    with open('C:\\Users\\czwat\\OneDrive\\Documents\\scores.txt', 'r') as f:
        lines = f.readlines()
        score = lines[0].strip()
...
def draw_window(surface, grid, score = 0, last_score = 0):
    ...
    #high score
    font = pygame.font.SysFont('freesanbold.ttf', 30)
    label = font.render(f'High Score: {str(last_score)}', 1, (255,255,255))
